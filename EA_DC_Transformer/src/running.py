...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    del aggr_metrics['reprs'] 
    del aggr_metrics['targets'] 
    for k, v in aggr_metrics.items():
...
class SupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        per_batch = {'target_masks': [], 'targets': [], 'predictions': [], 'metrics': [], 'IDs': [], 'reprs':[]}
        for i, batch in enumerate(self.dataloader):
            ...
            per_batch['reprs'].append(batch_repr.cpu().numpy())
            per_batch['targets'].append(targets.cpu().numpy())
            per_batch['predictions'].append(predictions.cpu().numpy())
            per_batch['metrics'].append([loss.cpu().numpy()])
            per_batch['IDs'].append(IDs)
        ...
        self.epoch_metrics['reprs'] = np.concatenate(per_batch['reprs'], axis=0)
        self.epoch_metrics['targets'] = np.concatenate(per_batch['targets'], axis=0).flatten()
