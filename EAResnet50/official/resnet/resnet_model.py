...
def asconv2d_fixed_padding(inputs, filters, kernel_size, strides, alpha, prev_atts,
                           data_format, do_downsample, k, v, Nh, relative, beta=0.5):
  dk = int(filters * k)
  dv = int(filters * v)
  ...
  conv_out = conv2d_fixed_padding(
      inputs=inputs, filters=filters - dv, kernel_size=kernel_size,
  ...
  if do_downsample:
    if data_format == "channels_first":
      _, _, orgH, orgW = shape_list(inputs)
    else:
      _, orgH, orgW, _ = shape_list(inputs)
    inputs = tf.nn.avg_pool2d(
            inputs, 3, 2, padding='SAME',
            data_format="NCHW" if data_format == "channels_first" else "NHWC")
  ...
  if do_downsample:
    attn_out = tf.image.resize_images(attn_out, (orgH, orgW),
...
def _bottleneck_asblock_v1(inputs, filters, training, projection_shortcut,
                           strides, do_downsample, prev_atts, data_format, block_idx,
                           offset_idx, alpha=1, k=0.25, v=0.125, Nh=8, relative=True):
  ...
  with tf.variable_scope("asconv_{}_{}_0".format(block_idx, offset_idx)):
    inputs = asconv2d_fixed_padding(
        inputs=inputs, filters=filters, kernel_size=3, strides=strides,
        alpha=alpha, prev_atts=prev_atts, data_format=data_format,
        do_downsample=do_downsample, k=k, v=v, Nh=Nh, relative=relative)
  inputs = batch_norm(inputs, training, data_format)
  inputs = tf.nn.relu(inputs)
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      for i, num_blocks in enumerate(self.block_sizes):
        ...
        if filters_out >= 512:
          do_downsample = True if filters_out == 512 else False
          inputs = _bottleneck_asblock_v1(inputs, filters, training,
                                          projection_shortcut, strides,
                                          do_downsample, prev_atts, data_format, i, 0)

          for j in range(1, blocks):
            inputs = _bottleneck_asblock_v1(inputs, filters, training, None, 1,
                                            do_downsample, prev_atts, data_format, i, j)
        else:
          inputs = _bottleneck_block_v1(inputs, filters, training,
                                        projection_shortcut, strides, data_format)

          for _ in range(1, blocks):
            inputs = _bottleneck_block_v1(inputs, filters, training, None, 1,
                                          data_format)
