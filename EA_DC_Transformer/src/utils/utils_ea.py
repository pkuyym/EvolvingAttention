...
def register_record(filepath, timestamp, experiment_name, model_type, batch_size, learning_rate, d_model, num_layers, kernel_size,
                                          hidden_size, isdilated,alpha,beta,k,  avg_best_epoch, avg_best_val_loss, avg_best_loss, final_metrics=None, comment=''):
    """
    ...
    row_values = [timestamp, experiment_name, comment, model_type, batch_size, learning_rate, d_model, num_layers, kernel_size, 
                                                             hidden_size, isdilated,alpha,beta,k, avg_best_epoch, avg_best_val_loss,  avg_best_loss] 
    ...
    if not os.path.exists(filepath):  # Create a records file for the first time
        ...
        header = ["Timestamp", "Name", "Comment", "Encoder","Batch_Size","LR", "D_model", "Num_layers","kernel_size", "Hidden_size", "IsDilated_for_Conv","alpha", "beta", "k", "Avg_Best_Epoch","Avg_Valid_Best_Loss", "Avg_Best_Loss"] 
        if final_metrics is not None:
